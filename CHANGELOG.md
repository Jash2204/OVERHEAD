...
## v8.0.1 — guide on every visit (June 2026)

The welcome guide now appears on **every** visit, not just the first. The old
`overhead-tour-done` flag is gone — nothing about the guide is stored, in keeping with
OVERHEAD's no-stored-data approach. It's still skippable (SKIP) and replayable any time
via the **?** button, top-left. When location access is denied, the ZENYTH prompt now
waits politely until the guide is closed, so the two never overlap.
