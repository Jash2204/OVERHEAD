...
projection maths and the sacred pure-black projector path are untouched.

### Concise, grouped control menu
The long flat list of toggles is gone. Sky layers now live one tap deep in five
grouped pop-overs — **Sky Objects · Overhead Traffic · View · Style · Projector** —
reusing the same pop-over pattern as Time-lapse Settings. ZENYTH, Open Sky Output
and Record stay as top-level primaries. Every toggle keeps its exact id and handler;
only its location in the menu changed, so the projector output window and its state
sync are unaffected. A group button lights up when any layer inside it is active, and
Escape closes any open panel.

### Feature tour trimmed
The onboarding tour was cut from 31 steps to 12, each pointing at a control that is
still visible (the group buttons and primaries), so nothing highlights a hidden item.

### Privacy & security hardening
- **Self-hosted fonts.** Share Tech Mono and Rajdhani are now served locally from
  `/fonts`; the page no longer imports from Google Fonts, so visitor IPs are no longer
  exposed to a third party (a GDPR improvement on the live site).
- **Locked-down data proxy.** `api/proxy.js` and `proxy.js` previously forwarded *any*
  `?url=`, which let strangers use the deploy as an open relay (SSRF / abuse). Both are
  now restricted to an allowlist of OVERHEAD's own data hosts (adsb.lol,
  Nominatim) and reject everything else with 403. They also send an identifying
  `User-Agent` (required by Nominatim's policy).
- **Edge caching.** Proxy responses now carry `Cache-Control`, so the edge serves many
  concurrent users in the same area from one upstream call — faster, cheaper, and gentle
  on the data providers' rate limits during traffic spikes. Short TTL for aircraft,
  long TTL for place names.

### Smoother live data
The live-aircraft feed's network fetch and JSON parse now run in a background **Web
Worker**, off the main render thread, so a slow download can't cause a frame hitch. It
falls back gracefully to the previous inline fetch where Workers/blob URLs aren't
available (e.g. `file://`), and to an inline retry if the worker ever returns empty.

### Location honesty
When location access is denied, the readout now reads **"Location not set"** instead of
showing the default city as if it were yours, and ZENYTH opens promptly to prompt for a
real one. The placeholder sky is clearly labelled as such.
...
OVERHEAD no longer depends on GPS. The new **ZENYTH** panel lets you set what's above
your head from anywhere:

- **Major cities** — teleport the night sky to any of 20 world cities. The destination
  is shown at its local solar midnight, so it's always a true night sky (never daytime).
- **World-class dark-sky sites** — ten of the planet's best stargazing locations
  (Mauna Kea, Atacama, Aoraki Mackenzie, NamibRand, La Palma, Wadi Rum, Salar de Uyuni,
  Cherry Springs, Exmoor, Jasper). Picking one auto-configures the most authentic view:
  pristine Bortle-1 darkness and realistic sky.
- **Your own coordinates** — type exact latitude/longitude and the sky stays *live*, the
  genuine sky above that spot right now. Coordinates are reverse-geocoded to a place name.
- **Location denied? Still works.** If GPS isn't shared, OVERHEAD opens ZENYTH on launch
  and invites you to set your spot — the whole point: it shows what's overhead, you just
  tell it where.

Remote dark-sky sites raise a brief notice that live air traffic will be sparse.
Southern-hemisphere location readouts now label °S correctly (was hardcoded °N).

**Architecture (backend-first).** Every position change now flows through a single
`applyLocation()` entry point, and the entire celestial sky reads one sky-clock
(`skyTimeOffsetMs`, consumed only by `getSiderealData`). The rendering engine is
unchanged — teleporting just feeds it a different instant. This is deliberate groundwork
for moving-vehicle (car-ceiling) positioning and a future time-of-day daylight model.
No location data is stored — ZENYTH is entirely session-only.
